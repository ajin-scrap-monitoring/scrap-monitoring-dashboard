...
function DashboardHeader({ activePage, initialNotifications }: { activePage: "monitoring" | "history" | "recordings" | "admin"; initialNotifications: HeaderNotification[] }) {
  ...
  return (
    <header className="topbar" ref={headerRef}>
      <a className="brand" href="/" aria-label="AJIN SCRAP MONITORING 홈"><span className="brand-mark" aria-hidden="true" /><span>AJIN SCRAP MONITORING</span></a>
      <nav className="global-nav" aria-label="주요 메뉴">
        <a className={`nav-link ${activePage === "monitoring" ? "active" : ""}`} href="/" aria-current={activePage === "monitoring" ? "page" : undefined}>현황</a>
        <a className={`nav-link ${activePage === "history" ? "active" : ""}`} href="/history" aria-current={activePage === "history" ? "page" : undefined}>이력</a>
